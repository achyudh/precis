...
class ConvPathAttn(nn.Module):
    ...
    def forward(self, source_subtoken_indices, node_indices, target_subtoken_indices, source_subtoken_lengths,
        ...
        source_subtoken_embed = self.token_embedding(source_subtoken_indices)  # (batch, max_contexts, max_subtokens, subtoken_embedding_dim)
        target_subtoken_embed = self.token_embedding(target_subtoken_indices)  # (batch, max_contexts, max_subtokens, subtoken_embedding_dim)
        node_embed = self.path_embedding(node_indices)  # (batch, max_contexts, max_path_nodes, node_embedding_dim)
